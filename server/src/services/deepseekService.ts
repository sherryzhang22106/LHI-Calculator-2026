...
重要：每个###后必须有独立内容，不要合并！`;
  ...
  static async generateASAReport(data: GenerateASAReportRequest): Promise<{ success: boolean; report?: string; error?: string }> {
    try {
      ...
      const prompt = `你是一位精通依恋理论（Attachment Theory）的资深心理咨询师，拥有20年以上的临床经验。请基于用户的测评结果，生成一份非常详细、深入的心理分析报告。
...
**报告要求：**
1. **长度**：8000-10000 字（必须详细完整，不可简略）
2. **详细结构**（使用 Markdown 格式）：
   
   **一、关系底色：核心模式深度解析**（2000-2500字）
...
请直接输出完整的8000-10000字报告正文，务必详细深入，不要包含任何前置说明或附加信息。`;
      ...
      try {
        const apiResponse = await fetch(this.API_URL, {
          ...
          body: JSON.stringify({
            ...
            temperature: 0.75,
            max_tokens: 28000,  // 支持8000-10000字的详细中文报告（约需25000+ tokens）
            top_p: 0.95
          }) as any,
